...
export const Route = createFileRoute("/industries")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sector-specialist legal counsel across energy, real estate, finance, technology, maritime, and immigration." },
    ...
    <>
      ...
      <section className="container-prose py-20">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {industries.map((i) => (
            <a key={i.slug} href={`/industries/${i.slug}`} className="group p-7 rounded-md border border-border bg-card hover:border-gold hover:shadow-card-soft transition">
              <div className="font-display text-xl font-semibold text-navy mb-2 group-hover:text-gold transition">{i.name}</div>
