...
type Field = { name: string; label: string; type?: string; required?: boolean; full?: boolean; options?: string[] };
...
export function ConsultationForm({
  ...
  const fields: Field[] =
    variant === "full"
      ? [
          ...
          { name: "matter", label: "Practice area", options: ["Cross-border representation", "Corporate / Commercial", "Real Estate", "Immigration", "Dispute Resolution", "Wills & Estates", "Other"] },
          { name: "message", label: "Briefly describe your matter", type: "textarea", required: true, full: true },
        ]
      : [
    ...
    return (
      <div className="rounded-md border border-border bg-card p-8 shadow-card-soft text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          We have received your request. A member of our team will reach out within one business day.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-md border border-border bg-card p-6 md:p-8 shadow-card-soft">
      <h3 className="font-display text-2xl font-semibold text-navy mb-1">{title}</h3>
      <p className="text-sm text-muted-foreground mb-6">All inquiries are confidential. We respond within one business day.</p>
