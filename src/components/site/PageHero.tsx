  ...
  return (
    <section className="relative bg-navy-gradient text-navy-foreground overflow-hidden">
      ...
      <div className="container-prose relative py-20 md:py-28">
        {breadcrumbs && (
          <nav aria-label="Breadcrumb" className="text-xs text-bone/60 mb-6 flex items-center flex-wrap gap-1.5">
            {breadcrumbs.map((b, i) => (
        ...
        <h1 className="font-display text-4xl md:text-6xl font-semibold leading-[1.05] max-w-4xl animate-fade-up" style={{ animationDelay: "60ms" }}>
          {title}
        </h1>
        {subtitle && (
          <p className="mt-6 max-w-2xl text-lg text-bone/75 leading-relaxed animate-fade-up" style={{ animationDelay: "120ms" }}>
            {subtitle}
          </p>
        )}
